...
class YOLOV8SegmentInfer:
    ...
    def draw_masks(self, img_src, masks, labels, colors, im_gpu, alpha=0.5):
        # 如果没有掩码就将图像转成numpy数组,直接返回
        if len(masks) == 0:
            img_src[:] = im_gpu.permute(1, 2, 0).contiguous().cpu().numpy() * 255
            return

        # 确保im_gpu和masks在同一个设备上，以避免设备不匹配的错误
        if im_gpu.device != masks.device:
            im_gpu = im_gpu.to(masks.device)

        # 过滤出需要绘制的掩码,如果类别是1，则保留这个掩码
        filtered_masks = [mask for label, mask in zip(labels, masks) if label == 1]

        # 将颜色数组转换为 PyTorch 张量并归一化
        # filtered_masks[0].device : 将colors张量移动到GPU上处理
        colors = torch.tensor(colors, device=filtered_masks[0].device, dtype=torch.float32) / 255.0
        colors = colors[:, None, None]
        im_gpu = im_gpu.flip(dims=[0])
        im_gpu = im_gpu.permute(1, 2, 0).contiguous()

        for i in range(0,len(filtered_masks)):
            torch_filtered_masks = torch.stack([filtered_masks[i]])
            torch_filtered_masks = torch_filtered_masks.unsqueeze(3)
            masks_color = torch_filtered_masks * (colors * alpha)
            inv_alpha_masks = (1 - torch_filtered_masks * alpha).cumprod(0)
            mcs = masks_color.max(dim=0).values

            if im_gpu.shape[2] != inv_alpha_masks[-1].shape[2]:
                im_gpu = im_gpu * inv_alpha_masks[-1] + mcs
            
            if im_gpu.shape[2] != mcs.shape[2]:
                mcs = mcs.permute(1, 2, 0).contiguous()
        
        im_mask_np = (im_gpu * 255).byte().cpu().numpy()
        img_src[:] = ops.scale_image(im_mask_np, img_src.shape)

        
        # 将过滤后的掩码转换为Tensor
        # filtered_masks = torch.stack([filtered_masks[1]])
        # print('')
        ...
        #     print('num='+str(i))
        #     torch_filtered_masks = torch.stack([filtered_masks[i]])
        #     print(len(torch_filtered_masks))
        #     print(torch_filtered_masks.shape)
        #     colors = torch.tensor(colors, device=torch_filtered_masks.device, dtype=torch.float32) / 255.0
        #     colors = colors[:, None, None]
...
if __name__ == '__main__':
    ...
    frame = cv2.imread('/home/gao/Desktop/yolov8/person_827.jpg')
